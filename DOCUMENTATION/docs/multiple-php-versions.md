---
slug: /multiple-php-versions
title: Multiple PHP Versions
description: Run more than one PHP version with Laradock, one version per project by switching PHP_VERSION, or two versions (for example 7.4 and 8.3) side by side in the same stack via the opt-in multi-php overlay.
keywords:
  - laradock multiple php versions
  - laradock php 7.4 and 8.3
  - two php versions docker
  - laradock php-fpm multiple versions
  - run legacy and new php together
  - laradock microservices php versions
---
...
This is the normal, fully-supported setup. See [Running Multiple Projects](/docs/multiple-projects) for how to keep several Laradock instances (or several sites in one instance) side by side without them clashing.
