...
Prefer MariaDB or PostgreSQL? Swap the name: `./laradock start nginx mariadb workspace` (or `docker compose up -d nginx mariadb workspace`). Joomla also runs on Apache if your theme relies on `.htaccess`: use `apache2` in place of `nginx`. The full catalog is [here](/docs/Intro#supported-services).
