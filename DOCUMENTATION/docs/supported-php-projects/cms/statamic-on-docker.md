...
That is genuinely all a fresh Statamic site requires. Everything below (a database, Redis, queues, the scheduler, search, mail) is an optional add-on with its own short section, so start only what your site actually uses. The full catalog is [here](/docs/Intro#supported-services).
