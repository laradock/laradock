...
Prefer MariaDB instead of MySQL, or Apache instead of NGINX (OpenCart ships an `.htaccess.txt` for Apache rewrites)? Swap the name: `./laradock start apache2 mariadb workspace`. The full catalog is [here](/docs/Intro#supported-services).
