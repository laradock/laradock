---
slug: /cubecart-on-docker
title: Run CubeCart on Docker
description: Run CubeCart on Docker in minutes with Laradock. What Docker gives a CubeCart store, why Laradock is the fastest way to get NGINX, PHP and MySQL running, and the exact commands, with any PHP version, without installing anything on your machine.
keywords:
  - cubecart on docker
  - run cubecart on docker
  - cubecart docker
  - cubecart docker setup
  - dockerize cubecart
  - cubecart docker environment
  - cubecart nginx mysql docker
---

## What is CubeCart?

[CubeCart](https://www.cubecart.com) is an open-source PHP shopping cart used by a large number of small merchants, known for being lightweight and simple to self-host. It is a plain PHP application, with no framework or Composer step in front of it, backed by a MySQL or MariaDB database and served through a web server via a browser-based installer.
...
Open [http://localhost](http://localhost) and run the CubeCart installation wizard. That is a full CubeCart store running on Docker.
