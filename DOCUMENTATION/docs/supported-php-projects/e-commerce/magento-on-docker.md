...
Prefer MariaDB or Elasticsearch instead? Swap the name: `./laradock start nginx mariadb elasticsearch workspace` (or `docker compose up -d nginx mariadb elasticsearch workspace`). The full catalog is [here](/docs/Intro#supported-services).
