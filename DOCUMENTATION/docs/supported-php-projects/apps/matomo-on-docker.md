...
The `-T` disables the pseudo-TTY so cron can run it non-interactively. In production you point `--url` at your real domain and schedule the same command; see [Prepare Laradock for Production](/docs/production#prepare-laradock-for-production).
...
The containers are production-style (real NGINX + PHP-FPM), so it is far closer to production than a native install. See [Prepare Laradock for Production](/docs/production#prepare-laradock-for-production) for the hardening steps, and remember to schedule Matomo's archiving cron the same way you would locally.
