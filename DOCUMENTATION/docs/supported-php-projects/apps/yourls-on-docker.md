...
The containers are production-style (real NGINX/Apache + PHP-FPM), so it is far closer to production than a bare native install. See [Prepare Laradock for Production](/docs/usage#prepare-laradock-for-production) for the hardening steps.
