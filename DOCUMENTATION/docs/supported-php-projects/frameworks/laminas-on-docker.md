---
slug: /laminas-on-docker
title: Run Laminas (Zend Framework) on Docker
description: Run any Laminas (formerly Zend Framework) app on Docker in minutes with Laradock. What Docker gives a Laminas project, why Laradock is the fastest way to get NGINX, PHP and MySQL running, and the exact commands, with any PHP version, without installing anything on your machine.
keywords:
  - laminas on docker
  - run laminas on docker
  - zend framework docker
  - laminas docker setup
  - dockerize laminas
  - laminas docker environment
...
Prefer to be asked? The optional [CLI](/docs/cli) detects Laminas and pre-selects nginx/mysql for you: `./laradock setup`, then `./laradock start`. It prints every real command it runs.
