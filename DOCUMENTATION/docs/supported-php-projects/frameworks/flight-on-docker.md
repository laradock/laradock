---
slug: /flight-on-docker
title: Run Flight on Docker
description: Run any Flight PHP app on Docker in minutes with Laradock. What Docker gives a Flight micro-framework project, why Laradock is the fastest way to get NGINX, PHP and a database running, and the exact commands, with any PHP version, without installing anything on your machine.
keywords:
  - flight php on docker
  - run flight php on docker
  - flightphp docker
  - flight framework docker setup
  - dockerize flight php
  - flight micro-framework docker
...
The containers are production-style (real NGINX + PHP-FPM), so it is far closer to production than PHP's built-in server. See [Prepare Laradock for Production](/docs/usage#prepare-laradock-for-production) for the hardening steps.
