...
Laradock's `workspace` runs cron when `WORKSPACE_INSTALL_CRON=true` is set in `.env` (rebuild the workspace after changing it). For production-style, always-on job runners, the dedicated `php-worker` container is the better home; see [Prepare Laradock for Production](/docs/production#prepare-laradock-for-production).
