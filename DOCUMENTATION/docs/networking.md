...
The same rule holds for every service: `postgres`, `mongo`, `elasticsearch`, and so on are each reachable at their own name from your app and from inside the `workspace`. Each service's own [service page](/docs/Intro#supported-services) lists its exact hostname and default port.
