---
slug: /services/open-webui
title: Open WebUI
description: Run Open WebUI in Laradock, a ChatGPT-style web interface for your local Ollama or LocalAI models, with no cloud account.
keywords:
  - laradock open webui
  - open webui docker
  - open webui docker compose
  - ollama web interface
  - chatgpt ui self hosted
  - local llm chat ui
---
...
[Open WebUI](https://openwebui.com) is a ChatGPT-style web interface for local models. [Ollama](/docs/services/ollama) gives you an API; Open WebUI gives you the chat window in front of it: pick a model, hold a conversation, upload a document to ask questions about, all in the browser with no cloud account and no per-token bill.
...
Refresh Open WebUI and `llama3.2` appears in the model picker. See [Ollama](/docs/services/ollama) for more on managing models.
...
`OPEN_WEBUI_LLM_ENGINE` is the service name Open WebUI talks to, so it retargets without editing any compose file. To use [LocalAI](/docs/services/localai) instead of Ollama, set both the engine and its port in your `.env`:
...
- **Replies are very slow.** The model runs on CPU unless a GPU is wired up, and larger models are slower. Try a smaller one (`llama3.2:1b`) or see [Ollama](/docs/services/ollama) for GPU notes.
- **Port 8322 already taken.** Change `OPEN_WEBUI_PORT` in your `.env`.

---

Need the model engine itself? See **[Ollama](/docs/services/ollama)** or **[LocalAI](/docs/services/localai)**. Routing to cloud providers too? **[LiteLLM](/docs/services/litellm)** puts one OpenAI-compatible endpoint in front of everything. New to Laradock? Start with **[Getting Started](/docs/getting-started)**.
