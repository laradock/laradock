...
title: Mosquitto
description: Run the Eclipse Mosquitto MQTT broker in Laradock. Start and stop the container, configure the port, back up and restore its persisted data, password-protect it, and connect with an MQTT client over WebSockets.
keywords:
  - laradock mosquitto
  - mosquitto docker
  - mqtt docker compose
