...
- **The `./laradock` CLI** — run Laradock without knowing Docker Compose: setup wizard, `start`, `workspace` shell, `doctor` health check, and more. → [CLI Reference](https://laradock.io/docs/cli)
- **A full local-AI stack** — Ollama, LocalAI, LiteLLM, and vector databases (pgvector, Qdrant, Weaviate, Chroma), plus modern runtimes like FrankenPHP, RoadRunner, and Laravel Reverb. → [Supported Services](https://laradock.io/docs/Intro#supported-services)
- **Production deploy** — take the same stack to a real server, a managed cloud, or Kubernetes with a hardened image. → [Deploy to Production](https://laradock.io/docs/production)
