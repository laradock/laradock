...
Source: https://laradock.io/docs/silex-on-docker
...
Need PostgreSQL instead? Swap the name: `./laradock start nginx postgres workspace` (or `docker compose up -d nginx postgres workspace`). The full catalog is [here](https://laradock.io/docs/Intro#supported-services).
...
When the app is ready, the same Laradock stack becomes your deployment. You build one hardened image, pinned to the exact PHP version the legacy app needs, and ship it to the host of your choice:
