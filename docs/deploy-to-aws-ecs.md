...
Source: https://laradock.io/docs/deploy-to-aws-ecs

Built your app with [Laradock](https://laradock.io/docs/Intro)? Ship it to **AWS ECS on Fargate** without changing your stack. `./laradock ship` builds one self-contained image (nginx + php-fpm, serving `$PORT`); ECS runs it as a task, no servers to manage.
...
Deploying elsewhere? The [full deploy guide](https://laradock.io/docs/production) covers every target, Kubernetes, Cloud Run, Fly, Render, and more.
