...
Source: https://laradock.io/docs/containers
...
Not listed? Browse the [full catalog of 100+ services](https://laradock.io/docs/Intro#supported-services).
...
Your data on disk is untouched either way, it lives under `DATA_PATH_HOST`, outside the containers (see [Data & Volumes](https://laradock.io/docs/volumes)). `docker compose down` additionally tears down the project's network, rarely something you need to think about for local dev.
...
For the common cases, toggling a bundled feature, installing a PHP extension, or adding a system package, see [Customizing Images](https://laradock.io/docs/customizing-images).
