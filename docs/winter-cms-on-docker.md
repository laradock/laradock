...
Source: https://laradock.io/docs/winter-cms-on-docker
...
Need Postgres instead of MySQL? Swap the name: `./laradock start nginx postgres redis workspace` (or `docker compose up -d nginx postgres redis workspace`). The full catalog is [here](https://laradock.io/docs/Intro#supported-services).
