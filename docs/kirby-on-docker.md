...
Source: https://laradock.io/docs/kirby-on-docker
...
The full catalog of every other available service is [here](https://laradock.io/docs/Intro#supported-services).
...
The containers are production-style (real NGINX + PHP-FPM), so it is far closer to production than a native install. Remember that production use still requires a paid Kirby license regardless of how you host it; see [Prepare Laradock for Production](https://laradock.io/docs/production#prepare-laradock-for-production) for the hardening steps.
