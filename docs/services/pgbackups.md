...
Source: https://laradock.io/docs/services/pgbackups
...
`pgbackups` is a companion sidecar container that takes scheduled, automatic backups of the [`postgres`](https://laradock.io/docs/services/postgres) service, built on [prodrigestivill/postgres-backup-local](https://github.com/prodrigestivill/docker-postgres-backup-local). It doesn't run a database of its own, it connects to your existing `postgres` container and dumps it on a schedule.
...
Need the database itself? See the **[Databases guide](https://laradock.io/docs/Intro#supported-services)** for Postgres. Back to the **[Getting Started guide](https://laradock.io/docs/getting-started)**.
