...
Source: https://laradock.io/docs/services/logstash
...
[Logstash](https://www.elastic.co/logstash) is the "L" in the ELK stack: a server-side data processing pipeline that ingests logs and events from multiple sources, transforms them, and ships them to a destination like Elasticsearch. Laradock builds it version-matched to **[Elasticsearch](https://laradock.io/docs/services/elasticsearch)** and **[Kibana](https://laradock.io/docs/services/kibana)** via the shared `ELK_VERSION` variable in the root `.env`.
...
Need the search/storage backend? See **[Elasticsearch](https://laradock.io/docs/services/elasticsearch)**. Need to visualize the ingested data? See **[Kibana](https://laradock.io/docs/services/kibana)**. New to Laradock? Start with **[Getting Started](https://laradock.io/docs/getting-started)**.
