...
Source: https://laradock.io/docs/moodle-on-docker
...
Prefer MariaDB or PostgreSQL? Swap the name: `./laradock start nginx mariadb workspace` or `./laradock start nginx postgres workspace` (or the `docker compose up -d ...` equivalent). The full catalog is [here](https://laradock.io/docs/Intro#supported-services).
