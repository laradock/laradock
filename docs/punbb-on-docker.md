...
Source: https://laradock.io/docs/punbb-on-docker
...
The containers are production-style (real NGINX + PHP-FPM), so it is far closer to production than a native install. See [Prepare Laradock for Production](https://laradock.io/docs/production#prepare-laradock-for-production) for the hardening steps, though given PunBB's maintenance status you should weigh whether it belongs in production at all.
