...
Source: https://laradock.io/docs/swoole-on-docker
...
Need MySQL or Redis for the app built on top of Swoole? Add them the same way as any other project: `./laradock start mysql redis` (or `docker compose up -d mysql redis`). The full catalog is [here](https://laradock.io/docs/Intro#supported-services).
...
When your app is ready, the same Laradock stack becomes your deployment. You build one hardened image of your app, with the matching Swoole build already baked in, and ship it to the host of your choice:
