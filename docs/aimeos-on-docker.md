...
Source: https://laradock.io/docs/aimeos-on-docker
...
(No Laravel app yet? Clone Laradock first, then [create one from the workspace container](https://laradock.io/docs/laravel-on-docker); Aimeos is added into it in the steps below.)
...
Prefer PostgreSQL or MariaDB? Swap the name: `./laradock start nginx postgres redis workspace` (or `docker compose up -d nginx postgres redis workspace`). The full catalog is [here](https://laradock.io/docs/Intro#supported-services).
