...
Source: https://laradock.io/docs/bolt-cms-on-docker
...
Prefer MySQL or PostgreSQL instead? Add the service: `./laradock start nginx mysql workspace` or `./laradock start nginx postgres workspace`. The full catalog is [here](https://laradock.io/docs/Intro#supported-services).
...
The containers are production-style (real NGINX + PHP-FPM), so it is far closer to production than a native install. For production, swap the default SQLite file for MySQL or PostgreSQL and see [Prepare Laradock for Production](https://laradock.io/docs/production#prepare-laradock-for-production) for the hardening steps.
