# Laradock vs the Official WordPress Docker Setup

Source: https://laradock.io/docs/laradock-vs-wordpress-docker
...
The `workspace` container ships WP-CLI, so `wp core download`, `wp config create`, and `wp core install` all work inside it. You get a real NGINX in front, Redis for object caching, and any PHP version from 5.6 to 8.5. Full walkthrough: **[Run WordPress on Docker](https://laradock.io/docs/wordpress-on-docker)**.
...
See the full landscape, including native GUI tools: **[Laradock vs Local WP](https://laradock.io/docs/laradock-vs-local-wp)** and **[Laradock vs Others](https://laradock.io/docs/laradock-alternatives)**. Ready to try it? **[Run WordPress on Docker](https://laradock.io/docs/wordpress-on-docker)** takes about five minutes.
