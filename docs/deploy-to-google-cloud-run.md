...
Source: https://laradock.io/docs/deploy-to-google-cloud-run

Built your app with [Laradock](https://laradock.io/docs/Intro)? Ship it to **Google Cloud Run** without changing your stack. `./laradock ship` builds one self-contained image (nginx + php-fpm) that listens on the `$PORT` Cloud Run injects, so it just works, and scales to zero when idle.
...
Deploying elsewhere? The [full deploy guide](https://laradock.io/docs/production) covers every target, Kubernetes, ECS, Fly, Render, and more.
