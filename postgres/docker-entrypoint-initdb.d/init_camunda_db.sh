...
if [ "$CAMUNDA_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $CAMUNDA_POSTGRES_USER WITH PASSWORD '$CAMUNDA_POSTGRES_PASSWORD';
		CREATE DATABASE $CAMUNDA_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $CAMUNDA_POSTGRES_DB TO $CAMUNDA_POSTGRES_USER;
	EOSQL
	echo
fi
